...
describe('[api] comments', () => {
  test('[fetchCommentsFromPost] capturando comentários de um post válido', () => {
    const ID_POST = '8xf0y6ziyjabvozdd253nd';
    const testCondition = (comments) => {
      expect(comments.length).toBeGreaterThanOrEqual(0);
    };

    return fetchCommentsFromPost(ID_POST).then(testCondition);
  });
  test('[fetchCommentsFromPost] capturando comentários de um post inválido', () => {
    const ID_POST = '0';
    const testCondition = (comments) => {
  ...
  test('[fetchComment] capturando comentário válido', () => {
    const ID = '894tuq4ut84ut8v4t8wun89g';
    const testCondition = (response) => {
      expect(response.id).toBe(ID);
    };

    return fetchComment(ID).then(testCondition);
  });
  test('[fetchComment] capturando comentário inválido', () => {
    const ID = '-1';
    const testCondition = (response) => {
